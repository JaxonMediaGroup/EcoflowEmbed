"""
create_club_de_golf.py - Populate the existing Club de Golf chatflow in
Flowise and register it in projects.json.

Usage:
    python create_club_de_golf.py
    python create_club_de_golf.py --dry-run
"""
...
TARGET_CHATFLOW_ID = "fdc8d4ea-5abe-44f7-bcef-56027ec591f1"
...
def save_project_entry(chatflow_id: str) -> None:
    config = load_config()
...
def build_payload(flow_data: dict) -> dict:
    chatbot_config = {
        "welcomeMessage": "Bienvenido a Villa Fairway dentro de Club de Golf La Esmeralda. Puedo ayudarte con precios, amenidades, financiamiento, ubicacion, inversion y visitas. Que te gustaria saber?",
        "botMessage": {"backgroundColor": "#f3f7f2", "textColor": "#24322a"},
        "userMessage": {"backgroundColor": "#12762F", "textColor": "#ffffff"},
        "textInput": {
            "backgroundColor": "#ffffff",
            "textColor": "#24322a",
            "sendButtonColor": "#12762F",
        },
        "chatWindow": {"backgroundColor": "#ffffff"},
        "poweredByTextColor": "#24322a",
        "footer": {"company": "Villa Fairway", "companyLink": ""},
    }
    return {
        "name": PROJECT_NAME,
        "type": "AGENTFLOW",
...
def main() -> None:
    ...
    if not os.path.exists(JSON_PATH):
        ...
        print("Run: python build_club_de_golf.py")
        sys.exit(1)
    ...
    print("\nClub de Golf Chatbot Creator")
    print(f"  File  : {JSON_FILE}")
    print(f"  Nodes : {len(nodes)}")
    print(f"  Edges : {len(edges)}")
    print(f"  Target: {TARGET_CHATFLOW_ID}")
    ...
    response = requests.get(
        f"{flowise_url}/api/v1/chatflows/{TARGET_CHATFLOW_ID}",
        headers=headers,
        timeout=120,
    )
    if response.status_code != 200:
        print(f"ERROR: Could not load target chatflow {TARGET_CHATFLOW_ID}: {response.status_code}")
        print(f"  {response.text[:500]}")
        sys.exit(1)

    current = response.json()
    print(f"  Current name     : {current.get('name')}")
    print(f"  Current deployed : {current.get('deployed')}")

    if dry_run:
        print("\n[DRY RUN] Validation passed. No changes uploaded.")
        return

    print(f"\nUpdating chatflow '{PROJECT_NAME}' in Flowise...")
    ...
    if update_response.status_code != 200:
        print(f"\nFailed to update chatflow: {update_response.status_code}")
        print(f"  {update_response.text[:500]}")
        sys.exit(1)

    print("\nChatflow updated successfully")
    print(f"  Name : {PROJECT_NAME}")
    print(f"  ID   : {TARGET_CHATFLOW_ID}")
    print(f"  URL  : {flowise_url}/agentcanvas/{TARGET_CHATFLOW_ID}")
    save_project_entry(TARGET_CHATFLOW_ID)
